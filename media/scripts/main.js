jQuery(document).ready(function(jQuery) {
	...
	//close primary navigation when clicking on close button
	jQuery('.close-nav-btn').on('click', function(){
		jQuery('.menu-icon').removeClass('is-clicked'); 
		jQuery('.site-header').removeClass('menu-is-open');
		jQuery('.site-navigation-wrapper').removeClass('is-visible').one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend',function(){
			jQuery('body').removeClass('overflow-hidden');
		});
	});
...
jQuery(document).ready(function(jQuery) {
	// 获取切换按钮
	var darkModeToggle = jQuery('#darkModeToggle');
	...
	// 初始应用 - 直接从系统偏好获取
	var initialIsDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
	applyDarkMode(initialIsDark);
	
	// 切换按钮点击事件（如果存在）
	darkModeToggle.on('click', function() {
		// 切换当前模式
		var isCurrentlyDark = jQuery('body').hasClass('dark');
